...
export default class HeatmapDecorator implements Decorator {
  ...
  private static _computeSquareRange(inRange: Range2d): Range3d {
    ...
    const offsetDir =
      yLen > xLen ? Point3d.create(1, 0, 0) : Point3d.create(0, 1, 0);
  ...
  private gridToImageBuffer(
    ...
    const thematicSettings = ThematicGradientSettings.fromJSON({
      ...
      customKeys: [
        ...
        {
          value: 0.5,
          color: ColorDef.computeTbgrFromComponents(0, 255, 0, 150),
        },
        {
          value: 0.75,
          color: ColorDef.computeTbgrFromComponents(255, 255, 0, 100),
        },
        {
          value: 1.0,
          color: ColorDef.computeTbgrFromComponents(255, 0, 0, 50),
        },
      ],
    });
  ...
  private _createGraphics(context: DecorateContext): RenderGraphic | undefined {
    if (0 === this._points.length) return undefined;
    ...
    if (undefined === imageBuffer) return undefined;
    ...
    const texture = IModelApp.renderSystem.createTextureFromImageBuffer(
      imageBuffer,
      context.viewport.iModel,
      textureParams
    );
    ...
    const material = IModelApp.renderSystem.createMaterial(
      materialParams,
      context.viewport.iModel
    );
